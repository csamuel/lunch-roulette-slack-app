...
import { respin } from '../../../service/game';
...
export default async (req: VercelRequest, res: VercelResponse) => {
  ...
  try {
    ...
    const { game: updatedGame, error } = await respin(gameId, spinnerToken, 'token');
    ...
    console.error('Error respinning:', error);
    res.status(500).json({ error: 'Failed to respin' });
  }
};
