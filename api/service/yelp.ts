...
export async function getRestaurant(restaurantId: string): Promise<Restaurant> {
  const response = await axios.get<Restaurant>(`https://api.yelp.com/v3/businesses/${restaurantId}`, {
    headers: {
...
export async function findRestaurants(address: string, radius: number, maxPriceDollars: string): Promise<Restaurant[]> {
  ...
  const requests = totalOffsets.map(async (offset) =>
    await axios.get<{ businesses: Restaurant[] }>('https://api.yelp.com/v3/businesses/search', {
      headers: {
        Authorization: `Bearer ${YELP_API_KEY}`,
      },
      params: {
